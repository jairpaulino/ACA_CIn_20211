...
getAStarPath = function(initialStation, finalStation, 
                        distConnection = distConnection_df, 
                        distReal = distReal_df, detailedPath = F){
  
  #initialStation = 6; finalStation = 7; 
  ...
  if(initialStation == finalStation){
    return("The Values 'initialStation' and 'finalStation' must  to be different!")
  }
  
  currentPoint = initialStation
  route = initialStation
  move = c(initialStation)
  distance = 0
  routeGuide = list()
  ...
  repeat{
    ...
    for(i in 1:length(adjacent)){#i=1
      cost[i] = getPath(c(route, adjacent[i])) + distReal[finalStation, adjacent[i]] 
